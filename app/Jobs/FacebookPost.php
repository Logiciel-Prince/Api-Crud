    ...
    public function handle()
    {
        try {
            ...
            if($event->data['imageName'])
            {
                ...
                $response = Http::attach('attachment',file_get_contents($image),$imageName)->post(env('GRAPH_API_URL').'me/photos?access_token='.$event->data['pagetoken'].'&message='.$event->data['data']['desc']);
